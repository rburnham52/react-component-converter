...
import { analyzeCva, analyzeAllCvaDefinitions, analyzeForwardRef, hasForwardRef, getForwardRefComponentName, analyzeProps, analyzeImports, usesCnUtility, extractJsxFromComponent, extractJsxForNamedComponent, } from './analyzers/index.js';
/**
 ...
 */
export async function parseComponent(code, options = {}) {
    const warnings = [];
    const errors = [];
    ...
    if (radixImports.length > 0) {
        warnings.push(`Component uses Radix UI primitives (${radixImports.map(i => i.source).join(', ')}). ` +
 ...
 */
function findAllComponentDefinitions(sourceFile, cvaConfigs) {
    const components = [];
    ...
    const exportStatements = sourceFile.getStatements().filter(s => s.getKind() === SyntaxKind.ExportDeclaration);
 ...
 */
function extractForwardRefInfo(declaration) {
 ...
 */
function findUsedCva(code, cvaConfigs) {
 ...
 */
function analyzePropsForComponent(sourceFile, componentName, cvaConfig) {
 ...
 */
function createComponentStructure(name, props, baseClasses, children) {
    const mitosisProps = {};
 ...
 */
export async function parseComponentFile(filePath, options = {}) {
