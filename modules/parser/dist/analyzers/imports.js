 ...
 */
export function analyzeImports(sourceFile) {
    const imports = [];
 ...
 */
function categorizeImport(source, namedImports, defaultImport) {
    // React imports
 ...
 */
export function usesCnUtility(sourceFile) {
    const imports = analyzeImports(sourceFile);
    // Check if cn is imported
    const hasCnImport = imports.some(imp => imp.namedImports?.includes('cn') || imp.defaultImport === 'cn');
    if (!hasCnImport) {
        return false;
    }
