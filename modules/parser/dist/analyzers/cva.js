 ...
 * ```
 */
export function analyzeCva(sourceFile) {
    // Find all cva() calls
 ...
 */
export function analyzeAllCvaDefinitions(sourceFile) {
    const configs = {};
    // Find all cva() calls
    const cvaCalls = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)
        .filter(call => {
 ...
 */
export function extractCvaSource(sourceFile) {
    const cvaCalls = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)
