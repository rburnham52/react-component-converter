 ...
 */
export function analyzeProps(sourceFile, cvaConfig) {
    const props = [];
 ...
 */
function extractPropsFromInterface(iface, props, cvaConfig) {
 ...
 */
function extractPropsFromTypeAlias(alias, props, cvaConfig) {
                    ...
                    if (typeLiteral) {
                        for (const member of typeLiteral.getMembers()) {
                            if (member.getKind() === SyntaxKind.PropertySignature) {
                                const propDef = extractPropertyDefinition(member, cvaConfig);
                                if (propDef) {
                                    props.push(propDef);
                                }
                            }
 ...
 */
function extractPropertyDefinition(prop, cvaConfig) {
 ...
 */
export function getPropsTypeName(sourceFile) {
    const propsInterface = sourceFile.getInterfaces()
        .find(iface => iface.getName().endsWith('Props'));
    if (propsInterface) {
