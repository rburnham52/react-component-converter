 ...
 */
export function createSvelte5RunesPlugin(options = {}) {
    const { useBindable = true, useState = true, useDerived = true, useEffect = true, } = options;
    return {
        name: 'svelte5-runes',
        order: 90, // Run late, after code generation
        postGenerate: async (code, component, context) => {
            if (context.target !== 'svelte') {
                return code;
            }
            let result = code;
            // Get shadcn metadata if available
            const shadcnMeta = context.metadata.shadcn;
 ...
 */
function transformExportLetToProps(code, shadcnMeta, useBindable) {
 ...
 */
function buildPropsInterface(props, shadcnMeta) {
    const lines = [];
 ...
 */
function transformLifecycleToEffect(code) {
    // onMount(() => {...}) → $effect(() => {...})
    // afterUpdate(() => {...}) → $effect(() => {...})
    let result = code;
    // Transform afterUpdate
    result = result.replace(/afterUpdate\s*\(\s*\(\)\s*=>\s*\{/g, '$effect(() => {');
    ...
    result = result.replace(/onMount\s*\(\s*\(\)\s*=>\s*\{/g, '$effect(() => {');
    // Remove lifecycle imports
    result = result.replace(/import\s*\{\s*(?:onMount|afterUpdate|onDestroy)(?:\s*,\s*(?:onMount|afterUpdate|onDestroy))*\s*\}\s*from\s*['"]svelte['"];?\n?/g, '');
    return result;
}
/**
 * Transforms <slot /> to {@render children?.()}
 */
function transformSlotToRender(code) {
 ...
 */
function addSvelteElementImports(code, shadcnMeta) {
    if (!shadcnMeta?.forwardRef)
        return code;
    const htmlType = getHtmlAttributesType(shadcnMeta.forwardRef.elementType);
...
//# sourceMappingURL=svelte5-runes-plugin.js.map